...
class FlutterOrchestrator:
    def __init__(self, project_path: str):
        ...
        self.build_provenance = BuildProvenance(str(self.project_path / "build_output"))
    ...
    async def build_app(self) -> Dict[str, Any]:
        ...
        self.build_provenance = BuildProvenance(
            str(self.project_path / "build_output")
        )
        self._log("Validando projeto...")
        self.build_provenance.record_decision("Iniciando validacao do projeto")
        validation = await self._validate_project()
        if not validation['success']:
            result['final_error'] = validation['error']
            self.build_provenance.record_step(
                "validacao", "failed", time.time() - start_time,
                error=validation['error']
            )
            self.build_provenance.save_json()
            return result
        self.build_provenance.record_step(
            "validacao", "completed", time.time() - start_time
        )
        dep_start = time.time()
        ...
        if not deps_ok:
            ...
            if not deps_ok:
                ...
                self.build_provenance.record_step(
                    "dependencias", "failed", time.time() - dep_start,
                    error="Falha apos tentativa de correcao"
                )
                self.build_provenance.save_json()
                return result
        self.build_provenance.record_step(
            "dependencias", "completed", time.time() - dep_start
        )
        ...
        for attempt in range(1, self.max_retries + 1):
            ...
            try:
                ...
                if build_result['success']:
                    ...
                    self.build_provenance.record_step(
                        f"build_attempt_{attempt}", "completed",
                        time.time() - attempt_start
                    )
                    if build_result.get('path'):
                        self.build_provenance.record_artifact(
                            build_result['path'], "APK"
                        )
                        self.build_provenance.verify_apk(build_result['path'])
                    break
                else:
                    result['attempts'].append({
                        'attempt': attempt,
                    ...
                    self.build_provenance.record_step(
                        f"build_attempt_{attempt}", "failed",
                        time.time() - attempt_start,
                        error=build_result.get('error', 'unknown')
                    )
                    self.build_provenance.record_error(
                        build_result.get('error', ''), f"Attempt {attempt}"
                    )
                    ...
                    if attempt < self.max_retries:
                        ...
                        if fix_result['success']:
                            ...
                            self.build_provenance.record_step(
                                f"fix_attempt_{attempt}", "completed",
                                time.time() - fix_start
                            )
                            self.build_provenance.record_decision(
                                f"Correcao aplicada apos attempt {attempt}",
                                fix_result.get('summary', '')
                            )
                            continue
                        else:
                            self.build_provenance.record_step(
                                f"fix_attempt_{attempt}", "failed",
                                time.time() - fix_start,
                                error="Correcao nao teve sucesso"
                            )
                ...
                self.build_provenance.record_step(
                    f"build_attempt_{attempt}", "errored",
                    time.time() - attempt_start,
                    error=str(e)
                )
            finally:
        ...
        self.build_provenance.finalize()
        self.build_provenance.save_json()
        self.build_provenance.save_markdown()
