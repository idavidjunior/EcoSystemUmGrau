...
from orchestrator.build_provenance import BuildProvenance
...
class FlutterBuildOrchestrator:
    ...
    def __init__(self, project_path: str,
        ...
        self._build_provenance = BuildProvenance(str(self.output_dir))
        self._verify_apk_enabled = False
        self._learn_from_build_enabled = True
    ...
    # Padrao: LER goal-oriented loop + Android Pure SDK verification
    # Cada etapa: EXECUTAR -> VALIDAR -> COLETAR EVIDENCIA -> APRENDER
    ...
    def orchestrate(self, skip_tests: bool = False, debug: bool = False,
        ...
        # Reinicia proveniencia para este build
        self._build_provenance = BuildProvenance(str(self.output_dir))
        start_build = datetime.now()

        steps = [
            ("Pre-requisitos", lambda: self.check_prerequisites()),
            ("Validacao", lambda: self.validate_flutter_project()),
            ("Auto-revisao do Orchestrator", lambda: self._ai_self_review()),
            ("Dependencias", lambda: self.get_dependencies()),
            ("Correcao pre-build",
             lambda: self._apply_pre_build_fixes()),
            ("Analise", lambda: self.analyze_code()),
            ("Testes", lambda: self.run_tests(skip=skip_tests)),
            ("Build APK",
             lambda: self.build_apk(release=not debug, build_number=build_number)),
            ("Verificacao APK",
             lambda: self._verify_built_apk(apk_path) if apk_path else False),
            ("Artifacts", lambda: self.copy_artifacts()),
            ("Aprendizado", lambda: self._learn_from_build(apk_path)),
        ]
        ...
        for idx, (name, fn) in enumerate(steps):
            if self._cancelled:
                self.log("Build cancelado pelo usuario", "WARNING")
                self._build_provenance.record_step(name, "cancelled", 0)
                self._build_provenance.save_json()
            ...
            self.log(f"\n>>> {name}", "STEP")
            self._build_provenance.record_decision(f"Iniciando: {name}")
            try:
                result = fn()
                duration = time.time() - step_start
                if result is False and name != "Testes" and name != "Aprendizado":
                    self.log(f"Falhou em: {name}", "ERROR")
                    self._build_provenance.record_step(
                        name, "failed", duration,
                        error=f"Step retornou False"
                    )
                    self._build_provenance.save_json()
                    self.generate_build_report(apk_path, False)
                    return False
                if name == "Artifacts" and result:
                    apk_path = result
                    self._build_provenance.record_artifact(apk_path, "APK")
                if name == "Verificacao APK" and apk_path:
                    self._build_provenance.verify_apk(apk_path)
                self._build_provenance.record_step(name, "completed", duration)
                self._progress(int(((idx + 1) / total) * 100),
                               f"{name} — concluido")
            except Exception as e:
                duration = time.time() - step_start
                self.log(f"Erro em '{name}': {e}", "ERROR")
                self._build_provenance.record_step(name, "errored", duration, error=str(e))
                self._build_provenance.record_error(str(e), f"Step: {name}")
                self._build_provenance.save_json()
                self.generate_build_report(apk_path, False)
                return False
        ...
        self._build_provenance.finalize()
        self._build_provenance.save_json()
        self._build_provenance.save_markdown()
        self.generate_build_report(apk_path, ok)
        if ok:
            self.log("BUILD CONCLUIDO COM SUCESSO!", "SUCCESS")
            try:
    ...
    def _verify_built_apk(self, apk_path) -> bool:
        if not apk_path or not os.path.exists(apk_path):
            self.log("APK nao encontrado para verificacao", "WARNING")
            return False
        self.log(f"Verificando APK: {apk_path}", "INFO")
        result = self._build_provenance.verify_apk(apk_path)
        if result.get("valid"):
            self.log(f"APK VERIFICADO: {result.get('sha256', '')[:16]}... "
                     f"{result.get('size_mb', 0)}MB", "SUCCESS")
        else:
            warnings = [c for c in result.get("checks", [])
                        if c["passed"] is False]
            if warnings:
                self.log(f"APK com alertas: {len(warnings)} verificacoes",
                         "WARNING")
        return True

    def _learn_from_build(self, apk_path) -> bool:
        if not self._learn_from_build_enabled:
            return True
        try:
            error = ""
            if not apk_path:
                error = "Build falhou - nenhum APK gerado"
            build_log = "\n".join(
                e.get("message", "") for e in self.build_log[-50:]
            )
            self.kb_learner.learn_from_build(
                build_log, error, None, apk_path is not None
            )
            stats = self.kb_learner.get_stats()
            self.log(f"Learning: {stats['successful_builds']}/{stats['total_builds']} "
                     f"builds OK ({stats['learning_rate']*100:.0f}% taxa)", "INFO")
            return True
        except Exception as e:
            self.log(f"Erro ao aprender: {e}", "WARNING")
            return True
...
Exemplos:
  ...
  %(prog)s ./meu_projeto_flutter --build-number 42 --auto-install
    ...
    parser.add_argument("--verify-apk", action="store_true",
                        help="Verificar APK apos build (apksigner verify)")
