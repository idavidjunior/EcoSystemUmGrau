...
.DESCRIPTION
    Instala OpenCode, clona o repositorio em Desktop/Codigos/EcoSystemUmGrau/,
    configura skills unificadas, vault, MCP, variaveis de ambiente,
    chaves de API com validacao, e watcher.
    Um comando, zero config previa.
.PARAMETER VaultPath
    Caminho para o vault Obsidian (default: ~/Desktop/Codigos/EcoSystemUmGrau)
.PARAMETER InstallDir
    Onde clonar o repositorio (default: ~/Desktop/Codigos/EcoSystemUmGrau)
.PARAMETER Branch
...
if (-not $isAdmin) {
    Write-Host "`nReiniciando como Administrator para configurar em nivel Machine..." -ForegroundColor Yellow
...
function CheckSuccess($desc) {
    if ($LASTEXITCODE -and $LASTEXITCODE -ne 0) { Fail $desc; exit 1 }
    Ok $desc
}
...
function Test-ApiKey_Nvidia($key) {
    try {
        ...
        $result = Invoke-RestMethod -Uri "https://integrate.api.nvidia.com/v1/models" -Headers $headers -Method Get -TimeoutSec 10 -ErrorAction Stop
        if ($result -and $result.models) { return $true, "Encontrados $($result.models.Count) modelos disponiveis" }
        return $true, "API respondeu (modelos viaveis)"
    } catch {
        $code = [int]$_.Exception.Response.StatusCode
        if ($code -eq 401) { return $false, "Chave invalida (HTTP 401)" }
        if ($code -eq 403) { return $false, "Chave invalida (HTTP 403)" }
        if ($code -eq 429) { return $false, "Rate limit (HTTP 429)" }
...
function Test-ApiKey_OpenAI($key) {
    try {
        $headers = @{ Authorization = "Bearer $key" }
        $result = Invoke-RestMethod -Uri "https://api.openai.com/v1/models" -Headers $headers -Method Get -TimeoutSec 10 -ErrorAction Stop
        if ($result -and $result.data) {
            $gptModels = ($result.data | Where-Object { $_.id -match "gpt" }).Count
            return $true, "Encontrados $gptModels modelos GPT"
        }
        return $true, "API respondeu"
    } catch {
        $code = [int]$_.Exception.Response.StatusCode
        if ($code -eq 401) { return $false, "Chave invalida (HTTP 401)" }
        if ($code -eq 429) { return $false, "Quota excedida (HTTP 429)" }
        return $false, "Falha na conexao: $_"
    }
}
...
function Configure-ApiKey($name, $varName, $testFn) {
    ...
    if ($existing) {
        ...
        if ($choice -eq "S" -or $choice -eq "s" -or $choice -eq "") { Ok "Chave mantida"; return $true }
    }
    $key = Read-MaskedInput "Cole sua chave da $name e pressione Enter: "
    if ([string]::IsNullOrWhiteSpace($key)) { Warn "Nenhuma chave fornecida."; return $false }
    Write-Host "  Testando conexao..." -NoNewline
    $valid, $msg = & $testFn $key
    if ($valid) {
        Write-Host " OK!" -ForegroundColor Green; Ok "$($name): $msg"
        [Environment]::SetEnvironmentVariable($varName, $key, $EnvLevel)
        Set-Item -Path "Env:$varName" -Value $key
        Ok "Chave salva em variavel de ambiente (Machine)"; return $true
    } else {
        Write-Host " FALHOU" -ForegroundColor Red; Fail "$($name): $msg"; return $false
    }
}
...
foreach ($p in $prereqs) {
    try { $out = cmd /c "$($p.Cmd) 2>&1"; Ok "$($p.Name): $($out -join '')" }
    catch { Fail "$($p.Name) - Instale antes"; $allOk = $false }
}
if (-not $allOk) { exit 1 }
...
if ($oc) { Ok "OpenCode ja instalado em $($oc.Source)" }
else { Info "Instalando opencode-ai via npm..."; npm install -g opencode-ai; CheckSuccess "OpenCode instalado" }
...
# ============================================================
# PASSO 4: Chaves de API
# ============================================================
Step "4/6 - Chaves de API (plug and play)"
Write-Host "`nQuer configurar as chaves de API agora?" -ForegroundColor White -BackgroundColor DarkBlue
...
Write-Host "  4. Confirmar se esta funcionando`n"
...
if (Test-Path "$InstallDir\.git") {
    ...
    Push-Location $InstallDir; git pull origin $Branch; Pop-Location
} else {
...
Write-Host "========================================" -ForegroundColor Cyan
Write-Host "`nInstalado em:     $InstallDir" -ForegroundColor Gray
Write-Host "Vault em:         $VaultPath" -ForegroundColor Gray
Write-Host "Skills:           $InstallDir\skills\" -ForegroundColor Gray
Write-Host "Config OpenCode:  $env:USERPROFILE\.config\opencode\opencode.jsonc" -ForegroundColor Gray
Write-Host "`nComandos para comecar:" -ForegroundColor Cyan
...
if ($nvidiaOk -or $openaiOk) {
    Write-Host "`nChaves de API:" -ForegroundColor Green
    if ($nvidiaOk) { Write-Host "  [OK] NVIDIA_API_KEY" -ForegroundColor Green }
    if ($openaiOk) { Write-Host "  [OK] OPENAI_API_KEY" -ForegroundColor Green }
}
