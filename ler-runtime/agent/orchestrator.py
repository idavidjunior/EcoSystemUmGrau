...
class Orchestrator:
    def __init__(self, session, config):
        ...
        self.max_iterations = config.get("loop", {}).get("max_iterations", 1000)
        self.iteration = 0
    ...
    def _execution_loop(self):
        while self.iteration < self.max_iterations:
            ...
            # Stagnation detection — só para se max_iterations for atingido E não houver progresso recente
            progress = self.session.load_progress()
            completed_steps = len([s for s in progress.get("steps", []) if s.get("status") == "completed"])
            if completed_steps == self._last_progress_count:
                self._stagnation_count += 1
            else:
                self._stagnation_count = 0
                self._last_progress_count = completed_steps
            ...
            # max_iterations é segurança, não término
            if self._stagnation_count >= 30:
                self.session.log(f"[STAGNATION] No progress for 30 iterations. Breaking.")
                break
    ...
    def _phase_success_eval(self):
        ...
        if evaluation["passed"]:
            ...
            self.session.log("Score abaixo do threshold — replanejando autonomamente.")
            self.state.transition(AgentState.REPLANNING)
    ...
    def _phase_final_audit(self):
        ...
        if audit.get("all_checked"):
            ...
            self.session.log("Auditoria falhou — replanejando autonomamente.")
            self.state.transition(AgentState.REPLANNING)
    ...
    def _ask_user_feedback(self):
        try:
            print("\n" + "=" * 60)
            print("LER - CONFIRMACAO DE SUCESSO")
            print("=" * 60)
            print("O objetivo da missao foi alcancado com sucesso?")
            print("  [S] Sim — registrar como sucesso e encerrar")
            print("  [N] Nao — aprender com falha e reiniciar a missao")
            resposta = input("> ").strip().upper()
            return resposta == "S"
        except (EOFError, KeyboardInterrupt):
            return True

    def _learn_from_failure(self, feedback):
        try:
            progress = self.session.load_progress()
            experiencia = {
                "tipo": "user_reported_failure",
                "feedback": feedback,
                "iteracoes": self.iteration,
                "steps_completos": len(progress.get("completed_steps", [])),
                "steps_totais": len(progress.get("steps", [])),
            }
            self.learning_engine.learn_from_mission(experiencia)
            self.session.log("[FEEDBACK] Usuario reportou falha. Padrao registrado para aprendizado.")
        except Exception as e:
            self.session.log(f"[FEEDBACK] Erro ao aprender com feedback: {e}")

    def _finalize(self, reason):
        if reason == "completed":
            user_ok = self._ask_user_feedback()
            if not user_ok:
                self._learn_from_failure("Usuario reportou que objetivo NAO foi alcancado.")
                return self._restart_mission()
    ...
    def _restart_mission(self):
        self.session.log("=" * 60)
        self.session.log("REINICIANDO MISSAO COM BASE EM FEEDBACK DO USUARIO")
        self.session.log("=" * 60)
        self.state = AgentState()
        self.iteration = 0
        self._last_progress_count = 0
