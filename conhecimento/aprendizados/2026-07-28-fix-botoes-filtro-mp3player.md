...
### 1. Mudança de tema AppCompat → MaterialComponents quebrou os botões
...
- `layout_weight=1` dando ~68dp por botão → sobram ~28dp para texto
- "Favoritas" (9 chars a 11sp) precisa de ~50dp → só metade aparecia
...
3. **Teste de hipótese única por vez**: backgroundTint → inset → TextView
...
- **Ferramenta de diagnóstico:** `screencap` + Python/Pillow >> chutar病因
