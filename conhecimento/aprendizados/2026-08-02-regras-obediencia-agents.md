...
## Problema raiz
- `00-system-rules.md` não tinha frontmatter YAML → não era agente válido (só aparecia
...
## Lição
- Regras que devem valer SEMPRE → `AGENTS.md` (auto-carregado) + `instructions` no
  opencode.jsonc. Um arquivo em `config/agents/` sem frontmatter não garante nada.
- `mode: primary` criaria 2 agentes primários (conflito com Maestro) → usar `subagent`.
