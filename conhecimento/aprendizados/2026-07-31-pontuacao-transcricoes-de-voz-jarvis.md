...
- **STT é texto sem prosódia**: para pontuar fala, usar pistas LINGUÍSTICAS (palavras interrogativas, ordem verbo-inicial, padrões de pedido), não tentar analisar entonação que não existe no texto.
...
- **Saudação ≠ pergunta**: a classificação deve olhar DEPOIS da saudação ("Bom dia, que horas são?" — "que horas" decide).
