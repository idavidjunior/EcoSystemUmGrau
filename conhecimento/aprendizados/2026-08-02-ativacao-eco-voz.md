...
- **"Ativar Eco"** → confirmar "Eco ativado. Sistema de voz online." + responder
...
- **"Desativar Eco"** → confirmar "Eco desativado. Modo texto restaurado." +
  parar modo voz e finalizar `dialogo.py`
- **Persistência:** vale para QUALQUER sessão nova ou existente.

## Sincronização (3 camadas)

1. `config/agents/00-system-rules.md` (fonte única) — seção adicionada como CLÁUSULA PÉTREA
...
- `python scripts/preflight_check.py` → TODOS OS TESTES PASSARAM (6/6 MCP)
- `memory_engine.py stats` → 34 memórias ativas
