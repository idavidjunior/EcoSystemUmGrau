...
- Em `melhorar_fala()` (preparação do texto para o áudio), **antes** da troca de `:` por vírgula (que comeria o tempo):
  - `(\d{1,2}):00\b` → `\1 horas em ponto` (ex.: "22:00" → "22 horas em ponto")
  - `(\d{1,2}):(\d{2})\b` → `\1 horas e \2` (ex.: "21:44" → "21 horas e 44")
- Aplica-se apenas ao texto do ÁUDIO; a tela continua mostrando `21:44`.
