...
## Cláusula Pétrea — Fala Resumida (19/08/2026)
**O Jarvis fala em áudio apenas um resumo bem simples e curto do que está fazendo.**

1. Narre em 1 a 3 frases curtas o que vai fazer, o que está fazendo ou o que encontrou (ex.: "Vou compilar o app.", "Achei um erro no build.", "Pronto, instalado no celular.").
2. NUNCA fale detalhes longos, listas, etapas intermediárias, nomes de arquivos em sequência ou explicações técnicas extensas em áudio.
3. Detalhes só quando o usuário pedir explicitamente ("detalhe", "explica", "mostre mais", "por quê").
...
2. Dizer o que VAI fazer (plano/objetivo do comando) — em resumo curto.
3. Dizer o que ESTÁ fazendo no momento da execução — em resumo curto.
...
- `scripts/` — Scripts de operação do ecossistema
  ...
  - `unified_bridge.py` — Ponte única canônica (narrador desktop + TTS service + widget)
  - `narrador_desktop.py` — Narra em voz as respostas do opencode desktop
...
- `OPENCODE_SERVER_PASSWORD` (do .env)
...
Regra absoluta desde 30/07/2026: enviar "carru", "amãnhã", "julhiu" para o TTS é **proibido**. Thalita Neural já fala português nativamente.
...
**Como o Jarvis identifica PERGUNTA vs AFIRMAÇÃO no áudio:** como a bridge não "ouve" a entonação (não há dados de f0/durais do SpeechRecognizer), a classificação é **linguística**:
...
- `melhorar_fala()` expande horas digitais ANTES da troca de `:` por vírgula, apenas para o ÁUDIO:
  - `21:44` → `21 horas e 44`
  - `22:00` → `22 horas em ponto`
- `normalizar_hora_display()` converte formas faladas do LLM de volta para `HH:MM` no texto EXIBIDO:
  - `23 horas e 29` / `23h29` / `09 hs 30` / `23 e 29` → `23:29`
  - `22 horas em ponto` / `22h` → `22:00`
- TELA sempre em `HH:MM`; apenas o ÁUDIO fala por extenso (round-trip `text` → `gerar_audio`).
- Testes em `test_vox.py` (`teste_horas_para_fala()` + `teste_normalizar_hora_display()`).
...
- **Antes de agir:** avise o plano em resumo curto ("Vou fazer X, depois Y e Z").
- **Durante:** relate descobertas e bloqueios em voz, em resumo curto ("Encontrei tal coisa", "isso não funcionou, vou tentar assim").
- **Ao esperar por processos demorados (LLM, builds, testes):** envie um aviso intermediário de status ("Processando, já te respondo").
- **Ao concluir:** resuma o que foi feito e o resultado, em resumo curto.
- Regra permanente — nunca deixar o usuário aguardando sem informação.
- **A fala NUNCA detalha internals, nomes de arquivos em sequência ou etapas técnicas longas** — só quando o usuário pedir.
...
- **VoxUmGrau** (Android, Kotlin/Compose) — `Projetos/VoxUmGrau`
