...
# Find WIDGET_HTML start
start_marker = 'WIDGET_HTML = """<!DOCTYPE html>'
start = content.find(start_marker)
if start == -1:
    print("ERROR: WIDGET_HTML not found")
    exit(1)
...
print(f"Found WIDGET_HTML from {start} to {end} (length {end-start})")

# New HTML content
new_html = '''WIDGET_HTML = """<!DOCTYPE html>
...
end_marker = '</div>\n</div>\n<script>'
end = content.find('</div>\n</div>\n<script>', content.find('WIDGET_HTML = """<!DOCTYPE html>'))
...
# Build new content
new_widget_html = '''WIDGET_HTML = """<!DOCTYPE html>
<html><head><meta charset="utf-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0, maximum-scale=1.0, user-scalable=no">
<link rel="icon" href="jarvis.ico" type="image/x-icon">
<style>
...
# Do the replacement
start_idx = content.find('WIDGET_HTML = """<!DOCTYPE html>')
end_marker = '</div>\n</div>\n<script>'
...
# Do replacement
new_content = content[:start_idx] + new_widget + content[end_idx:]

with open('C:/Users/David Jr/Documents/Default Project/EcoSystemUmGrau/scripts/unified_bridge.py', 'w', encoding='utf-8') as f:
    f.write(new_content)

print("SUCCESS: WIDGET_HTML replaced!")
print(f"Replaced {end_idx - start_idx} chars with {len(new_widget)} chars")