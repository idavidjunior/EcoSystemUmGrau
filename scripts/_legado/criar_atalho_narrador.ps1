<# 
.SYNOPSIS
    Cria atalho "Narrador" na Area de Trabalho e/ou Menu Iniciar
    com icone personalizado para o narrador (TTS) do ecossistema.
#>
...
if ($Desktop -or (-not $StartMenu -and -not $Taskbar)) {
    ...
    $LNK_DESKTOP = Join-Path $DESKTOP_DIR "Narrador.lnk"
...
if ($StartMenu) {
    ...
    $LNK_START = Join-Path $START_DIR "Narrador.lnk"
...
Write-Host "`nUse: .\criar_atalho_narrador.ps1 -Desktop -StartMenu" -ForegroundColor Gray